...
def save_data(courses):
    """
    puts the courses in the list into 2 seperate files (one
    for ones with "CIST" in names, one for other courses)
    :param courses: list of courses
    :return: None
    """
    file_cist = open("cist-courses.txt", 'w')
    file_other = open("other-courses.txt", 'w')
    for course in courses:
        if "cist" in course.lower():
            file_cist.write(course.upper()+'\n')
        else:
            file_other.write(course.upper()+'\n')
    file_cist.close()
    file_other.close()
